...
オーケストレータ（台本、アプリ、あるいは LLM のループ）がコマンドを投げ、状態を読みます。
...
これは行間の無音をなくすためのものです。1 行送って待ってから次を送ると、次の行の合成にかかる時間がまるごと行間になります。レンダラは台詞が積まれた瞬間にその音声を要求するので、キューが 1 つしかないと準備する時間がないからです。

![同じ 3 行を、1 行ずつ送った場合とまとめて送った場合](../images/turn-ja.svg)

開発環境での実測で、別々に送ると行間が 1.2 秒、まとめて送ると 0.3 秒でした。ショットを行に載せられるので、4 つの異なる画角をまたぐ一続きの台詞が 1 リクエストで済みます。

答え全体を 1 つの `batch` に入れて、ショットは行に載せてください。合成の待ち時間を払うのは、その答えの最初の 1 行だけになります。

`stage` に書かなかった軸は、それまでの値を保ちます。`null` を書くと空になり、部屋なら残響なし、背景なら素の背景になります。コマンドラインではフラグを省略するのが前者、`--room ''` が後者です。
...
`GET /api/vocabulary` は、読み込み中のアバターに何を頼めるかを返します。これは宣言ではなく発見されたものです。表情の一覧はモデル自身のシェイプ群から、衣装はメッシュから引いているので、アバターが変われば中身も変わります。システムプロンプトに貼るのはこのオブジェクトです。呼び出し側に伝わるよう、原稿の記法もその中に入れてあります。

`GET /api/state` は分岐に使う値をひと通り、ポーリングに耐える軽さで返します。発話中かどうか、発話 id、積まれている数、感情ベクトル、出ている描き起こし表情と効果、保持中のプリセットとジェスチャ、そして直前の指先解決が各腕にどれだけ無理をさせたかを表す `strain` です。

指さしの角度は、語彙が告げる範囲に**あえて**制限していません。この範囲は解剖学的なもので、それを超えて手を伸ばすのはエラーではなく想定された結果です。腕は届く限りのところまで行き、strain の値は、狙いどおりに届いたのか近似しかできなかったのかを呼び出し側が知る唯一の手段になります。

状態と並んで、レンダラが演技ではなくレンダラ自身について報告する値が 3 つあります。

- `voice` — 直前の take の処理内容と音量。
- `tuning` — 調律の層がいま実際に動かしている値。遠隔のフェーダーを、最後に送られた値ではなく、効いている値の位置に描けます。
- `avatars` — このレンダラが読み込めるキャラクターの一覧。いま立っているアバターに何を頼めるか、とは別の問いです。

さらに、どのレンダラでもなく**サーバ自身**が持つ値が 4 つあります。開いているファイル、サーバが持つリスト、サーバが協調させる常設状態に関わるものです。

- `recording` — 書き込み中の録画と、ディスクに実際に落ちたバイト数。ないときは null です。サーバ自身の数字にしているのは、黙って止まったレコーダとまだ動いているレコーダが、録画しているページ側からは見分けられないためです。
- `airing` — レンダラが開始を報告し、まだ終了を報告していない発話。台詞そのものが入っています。どの行かを言うのが `state.turn` で、その行が何と言っているかを言うのがこれです。開始した行はその時点で `queue` から外れており、終わるまで履歴にも入りません。ここに入るのはキューを通った行だけで、`/api/command` へ直接送った `say` はキューを通らないため入りません。
- `paused` — キューが止まっているかどうか。[録画](recording.md)を参照してください。
