...
Builds the pages, starts the control server and — if this checkout has one — the speech sidecar, and opens the panel and the stage as two native windows. Quitting stops again everything it started.

![What `yarn shell` starts](../images/shell.svg)

It is a convenience, not a fourth surface. Everything inside it is the same three pages on the same loopback addresses, and everything it can do to the show it does through the same control API. What it adds is the handful of things a browser tab cannot do, and each of them is a question about the desk rather than about the broadcast.
...
## What it starts, and what it leaves alone
...
2. **The speech sidecar**, only when `tools/tts/.venv/bin/python` exists and nothing is already answering on the socket. It is started and never waited on — loading the model takes the better part of a minute, and the viewer and the control API work without a voice.
3. **The two windows**, on `/panel/` and `/monitor/`.

A control server **already running on this checkout is used rather than replaced**, so a `yarn dev` left up in a terminal is not taken down, and it is not stopped on quit either — the shell only ever stops a child it started itself.
...
That refusal is the point of the check. Two checkouts answer `/api/state` identically, so adopting the wrong one would give an operator two windows loaded from somebody else's build and a menu that opens *this* checkout's show directories while driving another — with no error anywhere. The failure would look like a renderer that will not come up.
...
Slower than the panel's own polling on purpose: this is three lines behind a menu somebody opens once something already seems wrong, not a surface anybody reads during a broadcast. `Renderers` counts every attached renderer, which is at least the stage window — it is not a claim that OBS is live, because OBS has no distinct identity on the control protocol.

`Speech: absent` means no sidecar was found, which is the ordinary state of a checkout that has not run `make voice`. `down` means one answered earlier and has stopped, which is the one worth acting on: the queue still drains and the mouth still moves, so a voice that died mid-broadcast is otherwise invisible. See [Speech](speech.md).
...
It costs a reload: the mute is on the URL — `/monitor/?mute=1` — rather than sent to a page that is already up, so the model loads again and the picture is black for a second or two. That is the right trade here and would not have been for the panel's preview. This is decided once while the desk is being set up, not reached for during a broadcast. See [Whether a renderer makes a sound](surfaces.md#the-renderers-url).
...
| The sidecar's output | `speech.log`, beside it | Where a model that failed to load says so |

Those two log files are the answer to "there is no sound" and "the renderer never came up". Throwing the children's output away was the earlier arrangement and left nothing anywhere to read.

## What the windows are allowed to do
...
- **One permission.** Everything is refused except sanitised clipboard *writes*, and that one only because taking the address bar away is what made it necessary: the panel composes a URL for OBS, and a button is the only way to get it out of a window with nothing to select from. Reading the clipboard is something no page here has any business doing. Device permissions are refused wholesale.
...
Four Chromium switches are set before the application becomes ready: three that stop a backgrounded or occluded window being throttled — a stage behind the panel has to keep rendering — and the autoplay policy that lets the stage make a sound without a click.
...
## It is deliberately not packaged

`yarn shell` runs out of a development checkout, and the shell resolves the repository from its own source file. There is no `.app`, no installer and no auto-update, and that is not a gap waiting to be filled: packaging this would put a purchased avatar inside a distributable, which is the one thing the licences do not allow. See [Avatars](avatars.md#licensing-is-why-the-runtime-is-loopback-only).
