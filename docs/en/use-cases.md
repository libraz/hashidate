...
Eight things this runtime is set up to do. Each one is the same runtime with a different caller in front of it.
...
| [8](#8-checking-a-newly-rigged-model) | Checking a newly rigged model | An operator at a terminal |

## 1. An AI VTuber that answers chat

The common case, and the one the boundary is drawn for. The caller reads the chat, asks a model what to say, and hands the answer over. hashidate does the rest.
...
Send a whole answer as one batch rather than a line at a time: the gap between lines is 1.2 s when they travel separately and 0.3 s when they travel together. See [Send a whole answer at once](control-api.md#send-a-whole-answer-at-once).

## 2. Commentary over a game capture

The character is drawn on top of a game capture with a transparent background, and OBS does the compositing.
...
A PDF goes up behind the character, who presents from it. Page turns can ride on the lines, so the deck follows the script rather than an operator.
...
The deck is also readable as text, which is what the feature is for: a model that can read the deck can write the script for it and put the page numbers on the lines. See [Slides](slides.md).

## 4. A scripted segment, with no model at all

Nothing here requires a language model. A shell script is enough of an orchestrator, and it is the quickest way to rehearse a segment or reproduce a bug.
...
The other shape a scripted segment takes: not broadcast, recorded once. The panel lists what is in `show/scripts/` at the top of its Queue tab, and pressing one loads the run **without starting it** — the lines appear underneath, and the shot gets framed against a queue whose audio is already being synthesised. Record, on the Recording tab, then starts the take and releases the queue, in that order, so the file opens on the first word rather than on the wait for it.

The take stops itself a moment after the last line and the mp4 is in `show/recordings/`. The renderer that is not muted is the one that writes it, which on an ordinary desk is the stage rather than the panel's preview.
...
## 8. Checking a newly rigged model
...
`strain` in the state reports what the last fingertip solve cost each arm, which distinguishes an aim that was met from one the arm could only approximate. See [The control API](control-api.md#what-comes-back).
