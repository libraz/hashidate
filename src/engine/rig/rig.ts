...
/**
 * Elbow positions tried when solving a fingertip target, and how strongly the
 * search prefers to stay where it was last frame.
 *
 ...
 * position for strain is what turns "put the fingertip here" into a pose an arm
 * would actually adopt — it is the whole of the back-solve, and the reason the
 * anatomy model needs a cost rather than a pass/fail limit.
 *
 ...
 *
 * The continuity term breaks ties between the two shallow minima most targets
 * have — elbow out and elbow down — so the arm does not flip between them as a
 * target drifts past the midpoint. It has to be *small*. At 0.35 it was not a
 * tiebreaker but the deciding vote: pointing straight ahead, the raised-elbow
 * solution cost 0.03 in strain and the natural one sat half a turn away, where
 * the continuity term charged it 0.7. The arm pointed with its elbow in the
 * air, and every joint reported comfortable, because every joint was.
 */
const SWIVEL_SAMPLES = 24;
const SWIVEL_INERTIA = 0.05;

/**
 * How much better a different elbow has to be before the arm moves to it.
 *
 * Cost units. Sized against what actually perturbs a held pose: the vertical
 * bob a gesture writes into its target moves the cost of either solution by a
 * few tenths, and a real change of intent moves it by several whole units.
 */
const SWIVEL_SWITCH = 1.0;
...
export class Rig {
  ...
  /**
   * Find the least strained place on the elbow circle, and solve there.
   *
   ...
   * circle is sampled, scored by the anatomy model, and the cheapest wins.
   *
  ...
  private searchSwivel(
    ...
    // Hysteresis, which is not the same thing as the continuity weight above.
    //
    // Most face-height targets have two solutions — elbow hanging below the
    // hand, elbow cocked above it — and the cost surface between them is nearly
    // level. A weight that merely biases the score cannot fix that: too small
    // and the two swap places whenever anything nudges the balance, too large
    // and it stops being a tiebreaker and starts choosing the pose.
    //
    // So the choice is sticky instead. Staying where the elbow already is wins
    // unless the alternative is better by a real margin, and the margin is
    // wide enough that the small vertical bob written into a gesture cannot
    // reach it. Without this the cat-paw pose ran 165 degrees of elevation, then
    // 82, then 140, while merely being *held* — the arm visibly snapping between
    // two poses the search could not choose between.
    const prev = this._swivel[side];
    const stayA = refine(prev, score(prev));
    const stayC = score(stayA);
    if (Number.isFinite(stayC) && stayC - bestC < SWIVEL_SWITCH) {
      bestA = stayA;
      bestC = stayC;
    }
    ...
    const settled = prev + d * SWIVEL_TRACK;
   ...
   * elbow is not given: it is searched for, scored by the anatomy model, and the
   * least strained position on the circle wins. That search is the "natural"
   * part; without it a fingertip target has a whole circle of correct answers
   * and no reason to prefer the one a person would use.
   */
