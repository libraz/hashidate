...
キューに載せた台本を流しているところです。1 行が本番、19 行が待ち、どれにも演目が付いています。パネルが叩いているのは、オーケストレータが使うのと同じ制御 API です。
...
| **アバター** | 必須で、**同梱していません**。`src/avatars` の記述子が指す `public/models/<id>.glb` は git 管理外なので、クローンしただけでは描くものがありません。これまで動かしてきた 2 体は市販モデルで再配布できないため、モデルはご自身で用意し、`make glb` に通して、記述子を 1 ファイル足してください。 |
...
声の用意は、1〜2 分ぶんの WAV を `tools/tts/reference/clips/`（そのために空で同梱してあります）に置いて、`make voice` を一度走らせるだけです。声が無くても全部動きます。テキストから組み立てた尺で、無音のまま口だけが動きます。テストが走っているのもこの状態です。動くと知っておく価値はありますが、配信に出したい状態ではありません。

声を差し替えられるのは、モデルを差し替えられるのと同じ理由です。代わりのサイドカーは `POST /speak` で `{ "text": string }` を受けて `audio/*` を返し、`GET /health` で真偽値の `ready` を含む JSON に答えます。[音声](docs/ja/speech.md)を参照してください。
...
yarn ctl idle on                                # 自分で演じさせる。呼吸と視線は最初から動いている
...
hashidate はキャラクターを描いて動かすもので、AITuber そのものではありません。言語モデルも音声認識も配信の出力も持ちませんし、何を喋るかを決めるオーケストレータはこのリポジトリの外にいます。一線を越えたのは音声合成だけで、それも `tools/tts/` の範囲に留めてあります。HTTP 越しに呼ぶだけで import はしないサイドカーです。
