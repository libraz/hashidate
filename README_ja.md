...
キューに台本を載せた配信パネルです。1 行が本番、19 行が待機で、どれにも演目が付いています。パネルが叩いているのは、オーケストレータが使うのと同じ制御 API です。

## どのモデルにも依存しません

hashidate はアプリケーションではなくアダプタです。依存関係に提供元の SDK は入っておらず、設定する API キーもありません。人格もプロンプトも台本も会話の状態も持たず、どれも線のこちら側、呼び出し側のものです。線を越えて届くのは発話 1 つ、つまり台詞と、それを何で言うか（プリセット）と、どのショットで言うか（ステージ）だけです。それを受け取ってキャラクターが喋り、演じます。

![境界を越えるもの](docs/images/boundary-ja.svg)

モデルも提供元もフレームワークも差し替えられ、線の右側は何も変わりません。同じランタイムを、任意の言語で書いた LLM のループからも、MCP クライアントからも、モデルを一切使わないシェルスクリプトからも、配信パネルの前にいるオペレータからも動かせますし、1 回の配信の中で混ぜて使えます。
...
| **ゲーム画面の上での実況** | 背景を透過させてキャプチャの上に重ね、合成は OBS が行います。 | `?transparent=1&place=bottom-right:0.32x0.6` |
| **スライドを使った説明** | 後ろに PDF を出し、ページ送りを行に載せます。スライドはテキストとして読めるので、モデルがその台本を書けます。 | `deck`、`say --slide 2` |
| **台本どおりの進行** | モデルはどこにも要りません。シェルスクリプトでもオーケストレータとして十分です。 | `yarn ctl say …` |
...
![デモ資料の 1 ページと、同じフレームの隅に立つキャラクター](docs/images/slides.webp)

3 つめを実際に出したところです。資料はサーバがディスクから読む PDF で、キャラクターは同じフレームの隅に置かれ、ページ送りは台詞に乗って届きます。ブラウザソース 1 つで済み、OBS は何も合成しません。
...
| **アバター** | 必須で、**同梱していません**。`src/avatars` の記述子が指す `public/models/<id>.glb` は git 管理外なので、クローンしただけでは描くものがありません。これまで動かしてきた 2 体は市販モデルでここから再配布できないため、モデルはご自身で用意し、`make glb` に通して、記述子を 1 ファイル足してください。 |
| **声** | 起動には要りませんが、配信するなら必要です。`uv` と Python 3.11 が要り、PyTorch だけで数 GB あります。声を複製する元の録音も実在する人物のもので、**同梱していません**。 |
| Blender、OBS | モデルを変換するときと、結果を配信に乗せるときだけ。 |

声の用意は、1〜2 分ぶんの WAV を `tools/tts/reference/clips/`（そのために空で同梱してあります）に置いて、`make voice` を一度走らせるだけです。声が無くても全体は動き、テキストから組み立てた尺で、無音のまま口だけが動きます。この経路はテストと開発のためのもので、配信で使うものではありません。

声を差し替えられるのは、モデルを差し替えられるのと同じ理由によります。代わりのサイドカーは `POST /speak` で `{ "text": string }` を受けて `audio/*` を返し、`GET /health` で真偽値の `ready` を含む JSON に答えます。[音声](docs/ja/speech.md)を参照してください。
...
ビューアが `127.0.0.1:5173`、制御 API が `127.0.0.1:8765`、音声サイドカーは環境が作ってあれば `tools/tts/.run/speech.sock` のソケットに立ちます。`yarn dev` なら前の 2 つだけで、`yarn shell` ならパネルとステージをネイティブのウィンドウで開き、3 つとも下で動かします。
...
yarn ctl idle on                                # アイドル時の自動モード。呼吸と視線は常に動いている
...
hashidate はキャラクターを描いて動かすもので、AITuber そのものではありません。言語モデルも音声認識も配信の出力も持ちませんし、何を喋るかを決めるオーケストレータはこのリポジトリの外にあります。例外は音声合成だけで、それも `tools/tts/` の範囲に留めてあります。HTTP 越しに呼ぶだけで import はしないサイドカーです。

ループバック限定であることも意図的です。`--host` フラグも CORS ヘッダもトンネルもありません。検証に使っているアバターは再配布が許されておらず、レンダラを外に出すのはコードの変更である前にライセンスの判断になるためです。
...
テストは GLB を読まず、コード上で合成のアバターを組み立てます。購入した 16 MB のモデルを必要とするテストは、それを買ったマシンでしか走らないためです。
